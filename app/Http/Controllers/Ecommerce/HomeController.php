...
class HomeController extends Controller
{
        ...
        foreach ($categories as $key => $categorie) {
            ...
            $group_categories_product->push([
                ...
                "products" => $products->map(function ($product) {
                    ...
                    return [
                        "id" => $product->id,
                        "title" => $product->cDescripcion,
                        "slug" => $product->cSlug,
                        "stock" => $product->nStock,
                        "price_soles" => $product->nPrecioPEN,
                        "price_usd" => $product->nPrecioUSD,
                        /* "discount_g" => $discount_g, */
                        "imagen" => env("APP_URL") . "storage/" . $product->cImagen,
    ...
    {
        $product = Producto::where("cSlug", $slug_product)->first();
        if (!$product) {
            return response()->json(["message" => 403]);
        }
        $product_relateds = Producto::where("id", "<>", $product->id)->where("categoria_id", $product->categoria_id)->orderBy("id", "asc")->get();
    ...
    public function config_initial_filter()
    {
        ...
        return response()->json(["categories" => $categories, "reviews" => $reviews]);
    }
}
